use crate::layout::{RenderScene, RenderItem};
use crate::theme::Theme;

/// Render a Layout to terminal text (Unicode box drawing).
pub fn render_term(ir: &crate::ir::Graph, theme: &Theme, config: &crate::config::LayoutConfig) -> String {
...
pub fn render_term_scene(scene: &RenderScene) -> String {
    ...
    let scale_x = if scene.width > 0.0 { cols as f32 / scene.width } else { 1.0 };
    let scale_y = if scene.height > 0.0 { rows as f32 / scene.height } else { 1.0 };
    ...
    for (x, y, w, h) in &rects {
        ...
        if sy < rows && sx < cols { grid[sy][sx] = tl; }
        if sy < rows && sw < cols { grid[sy][sw] = tr; }
        if sh < rows && sx < cols { grid[sh][sx] = bl; }
        if sh < rows && sw < cols { grid[sh][sw] = br; }
        for cx in (sx + 1)..sw {
            if sy < rows { grid[sy][cx] = h_wall; }
            if sh < rows { grid[sh][cx] = h_wall; }
        }
        for cy in (sy + 1)..sh {
            grid[cy][sx] = v_wall;
            grid[cy][sw] = v_wall;
